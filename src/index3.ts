...
const SPEED_MULT_GUI = gui
  .add(CONFIG, "SPEED_MULT")
  .name("base speed")
  .max(5)
  .min(0.25)
  .step(0.05);
const NOISE_MAGNITUDE_GUI = gui
  .add(CONFIG, "NOISE_MAGNITUDE")
  .name("noise mag")
  .max(10)
  .min(0.0)
  .step(0.01);

const NOISE_FREQUENCY_GUI = gui
  .add(CONFIG, "NOISE_FREQUENCY")
  .name("noise frequency")
  .max(1)
  .min(0.0)
  .step(0.01);

const MAX_RES_GUI = gui
  .add(CONFIG, "MAX_RES")
  .name("max res")
  .max(4096)
  .min(256)
...
canvas.addEventListener("touchmove", (e) => {
  ...
  mouse.buttons = e.touches.length;
});
...
let BB_H = BB_SIZE;
let BB_W = BB_SIZE;
const INITIAL_CONDITIONS = Array(BB_SIZE * BB_SIZE * 4)
  .fill(0)
  .map(() => (Math.random() > 0.9 ? 255 : 0));
const substrateTX = Array(3)
  .fill()
  .map(() =>
    regl.texture({
      width: BB_SIZE,
...
const start = () =>
  started++ > 0
    ...
    : (async () => {
        ...
        regl.frame(
          ({ tick, drawingBufferWidth, drawingBufferHeight, pixelRatio }) => {
            ...
            let n_BB_W = Math.ceil(
              window.innerWidth * pixelRatio * Math.min(1, CONFIG.MAX_RES / mn),
            );
            let n_BB_H = Math.ceil(
              window.innerHeight *
                pixelRatio *
                Math.min(1, CONFIG.MAX_RES / mn),
            );
            if (n_BB_W !== BB_W || n_BB_H !== BB_H) {
              substrateTX.forEach((t, i) =>
                substrate[i]({ color: t({ width: n_BB_W, height: n_BB_H }) }),
              );
              BB_W = n_BB_W;
              BB_H = n_BB_H;
            }
            if (mouse.buttons || CONFIG.ALWAYS_SPAWN || tt < 100) {
              for (let i = 0; i < BLOCK_SIZE; ++i) {
                ...
                BLOCK.data[4 * i + 2] =
                  0.25 * (Math.random() - 0.5) * mouse_v.y;
                BLOCK.data[4 * i + 3] =
                  0.25 * (Math.random() - 0.5) * mouse_v.y;
              }
            ...
            for (let j = 0; j < 2; j++) {
              ...
              if (tt % 2 == 0)
                regl({ framebuffer: SPRITES[0] })(() => {
                  ...
                  const parts = new Array(vals.length / 4)
                    .fill(0)
                    .map((x, i) => i)
                    .filter((x) => Number.isFinite(vals[x * 4]))
                    .map((x) => [
                      vals[x * 4],
                      vals[x * 4 + 1],
                      vals[x * 4 + 2],
                  ...
                  if (pixelBuffer.length > 1 && oldBuffer.length > 1) {
                    ...
                    let gs = pixelBuffer.map((x, i) => {
                      ...
                      const at =
                        Math.abs(
                          Math.acos(
                            (dot[3] * old[3] + dot[2] * old[2]) / len / lenO,
                          ),
                        ) * Math.sign(dot[2] * old[3] - dot[3] * old[2]);

                      const a = Math.pow(
                        ((old[0] - dot[0]) * BB_W) ** 2 +
                          ((old[1] - dot[1]) * BB_H) ** 2,
                        0.5,
                      ); //Math.abs(Math.acos((dot[3]*old[3]+dot[2]*old[2])/len/lenO))*Math.sign((dot[2]*old[3]-dot[3]*old[2]));
                      ...
                      let nw =
                        (a / (1000 / 10 / 1000)) *
                        Math.sign(dot[2] * old[3] - dot[3] * old[2]); //a/2/Math.sqrt(1-Math.pow((dot[3]*old[3]+dot[2]*old[2])/len/lenO,2))*Math.sign((dot[2]*old[3]-dot[3]*old[2]));//Math.sin(Math.abs(Math.acos((dot[3]*old[3]+dot[2]*old[2])/len/lenO)));//freq/len-1;//Math.log(freq+0.0000001);
                      const dotPTT =
                        oldData.lastTurn[0] * dot[2] +
                        oldData.lastTurn[1] * dot[3];
                      ...
                      if (dotPTT < 0) {
                        ...
                        const dotPTT2 =
                          oldData.lastTurn[1] * dot[2] -
                          oldData.lastTurn[0] * dot[3];
                        if (dotPTT2 >= 0) {
                      ...
                      const v = Math.pow(
                        2,
                        Math.log2(
                          1 / (ttt.reduce((ac, b) => ac + b, 0) / ttt.length),
                        ) / 2,
                      );
                      if (i === 0) {
                        // window.k=nw;
                      ...
                      return [
                        ...
                        dot,
                        {
                          ...
                          plastTurnTime: nPLTT,
                          lastTurn: nLT,
                          turnCycle: nTurnCycle,
                          turnDur: ttt,
                          phase: (GA / Math.PI / 2) * 0,
                          turnMarks,
                        },
                      ];
                    ...
                    const nn = (ng) => {
                      ...
                      return ng;//Math.pow(2, Math.floor(rN * 12) / 12) * 440;
                    };
                    // if (tt % 2 == 0)
                    const mnG = gs.findIndex(
                      (x, i) => x[0][1][0] > 0 && i >= gs.length / l,
                    );
                    mB = new Array(l).fill(0).map((x, i) => {
                      const o =
                        gs[Math.floor((i + 0.5) * ((gs.length - 1) / l))];
                      const oBF5 = gs[mnG][0][1][0]; //Math.floor((0.5)*(gs.length-1))][0][1][0];
                      const oBF1 = gs[Math.floor(1 * (gs.length - 1))][0][1][0];

                      let newB =
                        tt % 8 === 0 && tt % (8 * Math.pow(4, i % 2)) === 0;
                      return newB
                        ? [
                            o,
                            [
        ...
        function connectToDevice(device) {
          ...
          device.onmidimessage = function (m) {
            ...
            if (command === 144) {
              ...
              if (key % 6 === 4) NOISE_FREQUENCY_GUI.setValue(velocity / 127);
              if (key % 6 === 5)
